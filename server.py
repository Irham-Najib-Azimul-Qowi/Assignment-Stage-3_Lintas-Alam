...
class Config:
    ...
    MQTT_PORT = 1883
    MQTT_TOPICS = {
        "ip": "lintas_alam/ip",  # Sesuaikan jika topik berbeda
...
def send_to_ubidots(name: str, timestamp: Optional[int] = None) -> None:
    """Mengirim data absensi siswa dan mata kuliah ke Ubidots."""
...
def publish_attendance_to_mqtt(name: str, timestamp_str: str) -> None:
    """Mempublikasikan data absensi ke MQTT."""
    if state.mqtt_client:
        ...
        state.mqtt_client.publish(Config.MQTT_TOPICS["attendance_share"], json.dumps(data))
...
def open_door_and_display(name: str) -> None:
    ...
    send_to_ubidots("door_status", 1, timestamp)
    ...
    send_to_ubidots("door_status", 0, int(time.time() * 1000))
...
def on_connect(client: mqtt.Client, userdata, flags, rc, properties=None) -> None:
    ...
    if rc == 0:
        ...
        client.subscribe([(Config.MQTT_TOPICS["ip"], 0), (Config.MQTT_TOPICS["schedule"], 0)])
    ...
    try:
        ...
        if topic == Config.MQTT_TOPICS["ip"]:
            ...
            if "course" in data:
                ...
                state.current_course_name = data.get("course", "Tidak ada jadwal")
                logger.info(f"Jadwal mata kuliah: {state.current_course_name}, {start} - {end}")
            elif "person" in data:
...
def process_video_frame(frame: np.ndarray) -> Optional[np.ndarray]:
    """Memproses frame untuk deteksi wajah."""
    if frame is None:
        return None
    ...
    try:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if (name != "Unknown" and
                name not in state.last_detected or
                (current_time - state.last_detected.get(name, datetime.min)).total_seconds() > 60):
                if check_schedule(name, current_time):
                    ...
                    send_to_ubidots(name, int(current_time.timestamp() * 1000))
...
def update_attendance_table() -> pd.DataFrame:
    ...
    data = [
        {
            "Nama": name,
            "Status": state.attendance_status.get(name, {"status": "Tidak Hadir"})["status"],
            "Waktu": state.attendance_status.get(name, {"waktu": "-"})["waktu"],
            "Mata Kuliah": state.attendance_status.get(name, {"course": "Tidak ada jadwal"})["course"]
        }
